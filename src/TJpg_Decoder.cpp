...
// TJpg_Decoder is a singleton without
...
TJpg_Decoder::TJpg_Decoder(SketchCallback sketchCallback) : tft_output(sketchCallback)
{
    // Setup a pointer to this class for static functions
    thisPtr = this;
}
...
unsigned int TJpg_Decoder::jd_input(JDEC *jdec, uint8_t *buf, unsigned int len)
{
    ...
    // Handle SPIFFS input
    if (thisPtr->jpg_source == TJPG_FS_FILE) {
        // Check how many bytes are available
        len = std::min<int>(len, thisPtr->jpgFile.available());
